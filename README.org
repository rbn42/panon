...
Providing panon is installed in your home directory, you can add your own shader files to =~/.local/share/plasma/plasmoids/panon/contents/shaders/=. Panon can detect and load shaders in this folder. A visual effect may consists only a single shader file with its name ended with =.frag=. Or it consists a folder with 2 shader files, =image.frag= and =buffer.frag=, in it.
...
| iChannel2   | Buffer: Output of buffer.frag, has the same width and height of the applet.                                     |

To use =iChannel2=, you need to create a folder in =shaders/= like [[file:kde/plasmoid/contents/shaders/spectrogram][spectrogram/]]. Then rename the original shader file =image.frag=. And create another shader file =buffer.frag=, to produce the buffer data. =buffer.frag= can access the uniforms above as =image.frag= can. 
